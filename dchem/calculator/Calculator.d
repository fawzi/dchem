...
class NDistOps:DistOpsGen,DistOps{
    ...
    mixin(serializeSome("NDistOps","how the macroscopic distances are defined (periodic boundary conditions,...)",`
...
class NoSymmNeighLooper:SymmNeighLooperGen,SymmNeighLooper{
    mixin(serializeSome("NoSymmNeighLooper",`Normal symmetry looper for systems without special symmetry`,``));
    mixin myFieldMixin!();
    mixin printOut!();
    this(string fName=""){
        if (fName.length>0) {
...
class ContextLimiter:InputElement{
    ...
    mixin myFieldMixin!();
    mixin(serializeSome("ContextLimiter",`Limits the number of contexts (to avoid using too many resources), several methods can use the same limiter (and then the total number of their contexts is limited)`,
...
class ContextLimiterClient:Method{
    ...
    mixin myFieldMixin!();
    mixin(serializeSome("ContextLimiterClient",`Limits a Method to the maximal number of contexts given by the context limiter`,`contextLimiter : the limiter that constrain this method
