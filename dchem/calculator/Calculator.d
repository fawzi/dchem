...
class NDistOps:DistOpsGen,DistOps{
    ...
    mixin(serializeSome("dchem.NDistOps","how the macroscopic distances are defined (periodic boundary conditions,...)",`
...
class NoSymmNeighLooper:SymmNeighLooperGen,SymmNeighLooper{
    mixin(serializeSome("dchem.NoSymmNeighLooper",`Normal symmetry looper for systems without special symmetry`,``));
    mixin myFieldMixin!();
    mixin printOut!();
    this(string fName=""){
        if (fName.length>0) {
...
class ContextLimiter:InputElement{
    ...
    mixin myFieldMixin!();
    mixin(serializeSome("dchem.ContextLimiter",`Limits the number of contexts (to avoid using too many resources), several methods can use the same limiter (and then the total number of their contexts is limited)`,
    `maxActiveContexts : maximum number of active contexts (non binding)
...
class ContextLimiterClient:Method{
    ...
    mixin myFieldMixin!();
    mixin(serializeSome("dchem.ContextLimiterClient",`Limits a Method to the maximal number of contexts given by the context limiter`,`contextLimiter : the limiter that constrain this method
